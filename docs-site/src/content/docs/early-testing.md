...
it better. v0.21.0 reads Python, JavaScript, TypeScript, Go, Java, Rust, C#, Ruby, PHP, or
any mix.
...
The command below runs packaged v0.21.0. An editable checkout produces the same
...
Prefer a permanent install? `pipx install codemble==0.21.0`, then run `codemble` — no
uv needed.

Codemble opens your browser to an in-app picker — pick your project folder
there, or pass one directly:
`uvx --from codemble==0.21.0 codemble ./path-to-your-project`.
...
4. Follow two journey steps, switch Easy/Expert, and confirm your current step
   and cited source stay the same.
5. Return to the system, choose **Prove understanding**, and light it.
6. Quit, run the same command, and confirm the system remains lit.
7. Visit two or three systems and confirm their routes remain charted after
   restart.

...
it better. v0.17.0 reads Python, JavaScript, TypeScript, Go, Java, Rust, C#, or
any mix.
...
The command below runs packaged v0.17.0. An editable checkout produces the same
...
Prefer a permanent install? `pipx install codemble==0.17.0`, then run `codemble` — no
uv needed.

Codemble opens your browser to an in-app picker — pick your project folder
there, or pass one directly:
`uvx --from codemble==0.17.0 codemble ./path-to-your-project`.

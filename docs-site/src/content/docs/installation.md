---
title: Installation
description: Run Codemble v0.16.0 and configure optional narration.
---

:::note[One current version]
The packaged app, current source, screenshots, and these instructions all match
**v0.16.0**. [Download the wheel or source archive](/Codemble/download/) when
...
<ol class="cm-steps">
  ...
  <li>
    ...
    <div class="cm-step-body">
      ...
      <pre class="cm-step-cmd"><code>uvx --from codemble==0.16.0 codemble</code></pre>
    </div>
  </li>
</ol>
...
codemble==0.16.0`, then `codemble`. Plain `pip install codemble==0.16.0` also works inside a
virtual environment.
...
```bash
uvx --from codemble==0.16.0 codemble ./your-project
```
...
Run `codemble --version` to confirm v0.16.0. Contributors should follow the
